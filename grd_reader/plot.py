...
from qtpy.QtCore import QByteArray, QPoint, QPointF, QRect, QRectF, QSettings
from qtpy.QtGui import QCloseEvent, QScreen
from qtpy.QtWidgets import QApplication, QMainWindow, QStatusBar, QWidget

from . import CurveData, GraphData, read_grd
...
class Cursor:
    def __init__(self, parent: pg.PlotItem) -> None:
        ...
        self.format_string: str = (
            "{scaled_value:.{decimals}f}{unit_gap}{si_prefix}{unit}"
        )
    ...
    def move(self, point: QPointF) -> None:
        axes: dict[str, dict[Literal["item", "pos"], pg.AxisItem | tuple[int, int]]] = (
            self._parent.axes
        )
        self._crosshair_v_line.setPos(point.x())
        self._crosshair_h_line.setPos(point.y())
        self._cursor_balloon.setHtml(
            self.format(point.x(), axes["bottom"]["item"].labelUnits)
            + "<br>"
        ...
        if axes["left"]["item"].range[1] - point.y() < balloon_height:
            anchor_y = 0.0
        if (
            anchor_x != self._cursor_balloon.anchor.x()
            or anchor_y != self._cursor_balloon.anchor.y()
        ):
    ...
    def format(self, val: float, unit: str) -> str:
        ...
        if self.si_prefix and unit:
            ...
            parts.update(
                {"si_prefix": "", "scaled_value": val, "exp": exp, "mantissa": man}
            )
...
class Plot(QMainWindow):
    def __init__(self, parent: QWidget | None = None) -> None:
        ...
        self._mouse_moved_signal_proxy: pg.SignalProxy = pg.SignalProxy(
            cast(pg.GraphicsScene, self._plot.scene()).sigMouseMoved,
            rateLimit=10,
    ...
    def load_settings(self) -> None:
        ...
        self.restoreGeometry(
            cast(QByteArray, self.settings.value("geometry", QByteArray()))
        )
    ...
    def plot(self, filename: Path) -> None:
        ...
        for index, curve in enumerate(grd_data.curves):
            self._canvas.plot(
                curve[0], curve[1], name=curve.key or None, pen=pg.mkColor(index)
            )
