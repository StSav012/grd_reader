...
class GraphData:
    ...
    def __repr__(self) -> str:
        ...
        return os.linesep.join((
            ...
            f'comment:       {os.linesep.join(self.comment)}',
            f'names:         {self.names}',
            f'units:         {self.units}',
            '',
            'curves:',
        ...
        for line in lines:
            ...
            if reading_axes_description != -1:
                ...
                if reading_axes_description > 0:
                    data.names.append(line.split(maxsplit=10)[-1].strip().replace(' ', '_'))
                    data.units.append(line.split(maxsplit=10)[-2].strip() if len(line.split()) > 10 else '')
                reading_axes_description += 1
                continue
            ...
            if line.startswith('#START Time:'):
                ...
                data.curve.duration = float(times[1].replace(',', '.')) - float(times[0].replace(',', '.'))
                continue
            if line.startswith('#START Curve Legend '):
                data.curve.key = line.split(':', maxsplit=1)[1]
                continue
            if data and data.curve.curve_number and line.startswith(f'#START Curve {data.curve.curve_number:d}'):
            ...
            if line.startswith('#START Curve Data'):
                ...
                if data.curve.curve_number and line.startswith('#END Curve %d -' % data.curve.curve_number):
                    reading_data = False
                    continue
                if data.curve.data.shape[0]:
                    data.curve.data = np.row_stack((data.curve.data,
                                                    [float(item.replace(',', '.')) for item in line.split()]))
                else:
                    data.curve.data = np.fromiter((float(item.replace(',', '.')) for item in line.split()),
                                                  dtype=np.float64)
                continue
        # end for line ...
    # end with open ...
